...
InstallFiles() {
	...
	$download_tool_cmd cns https://cdn.staticaly.com/gh/miscdec/cns_install/main/linux_${machine}${softfloat} || Error "cns download failed."
	$download_tool_cmd cns.init https://cdn.staticaly.com/gh/miscdec/cns_install/main/cns.init || Error "cns.init download failed."
	...
	if type systemctl && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/cns.service https://cdn.staticaly.com/gh/miscdec/cns_install/main/cns.service || Error "cns.service download failed."
